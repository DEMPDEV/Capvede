...
function Register()
{
	...
	if(!empty($_POST['Email']) AND !empty($_POST['Name'])) {
		...
		if($errors) {
			...
			$query = "INSERT INTO `Users` (`Email`,`Name`,`Password`, `IP`) VALUES ('".mysqli_real_escape_string($con, $_POST['Email'])."','".mysqli_real_escape_string($con, $_POST['Name'])."','".md5(md5($_POST['Email']).$_POST['Password'])."', '".mysqli_real_escape_string($con, $IP)."')";
            $result = mysqli_query($con,$query);
            if($result) {
                $query="UPDATE `Users` SET `Activated` = '1' WHERE `Email`= '".mysqli_real_escape_string($con,$_POST['Email'])."'";
                $result = mysqli_query($con,$query);
                if($result) {
                    echo '<script type="text/javascript"> window.onload = function () { alert("You have been signed up!"); } </script>';
                    header('Location: index.php');
                }
				else {
                ...
                <div class="container">
                    <div class="row">
                        <form action="" method="post" class="subscribe-form">
							...
							<fieldset class="col-md-offset-4 col-md-3 col-sm-8">
                                <input type="text" id="txtfield" name="Name" placeholder="Your name">
                            </fieldset>
							<fieldset class="col-md-offset-4 col-md-3 col-sm-8">
